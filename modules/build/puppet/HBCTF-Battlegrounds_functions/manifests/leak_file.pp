define HBCTF-Battlegrounds_functions::leak_file($leaked_filename, $storage_directory, $strings_to_leak, $owner = 'root', $group = 'root', $mode = '0777', $leaked_from = '' ) {
  if ($leaked_filename != ''){
    ...
    if (defined(File[$path_to_leak])){
      ...
      file { $path_to_leak:
        ...
        group    => $group,
        mode     => $mode,
        content  => template('HBCTF-Battlegrounds_functions/overshare.erb')
      }
    }
  }
}
