class moinmoin_195::config {
  ...
  if $HBCTF-Battlegrounds_parameters['business_name'] {
    $raw_default_page = regsubst($HBCTF-Battlegrounds_parameters['business_name'][0], ',', '', 'G')  # Remove commas from co. names
  } else{
    $raw_default_page = $HBCTF-Battlegrounds_parameters['default_page'][0]
  }
  ...
  ::HBCTF-Battlegrounds_functions::leak_files{ 'moinmoin_195-image-leak':
