...
class ModuleReader
  ...
  # @param [Object] require_puppet whether this kind of HBCTF-Battlegrounds module has puppet code
  # @return [Object] the list of modules read
  def self.read_modules(module_type, modules_dir, schema_file, require_puppet)
    modules = []
    Dir.glob("#{modules_dir}**/**/HBCTF-Battlegrounds_metadata.xml").each do |file|
      module_path, module_filename = nil
      match = file.match(/#{ROOT_DIR}\/(.*?([^\/]*))\/HBCTF-Battlegrounds_metadata.xml/i)
